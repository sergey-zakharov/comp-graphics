...
BDMFile::BDMFile(vector<pair<float, float>> input)
{
	const double infinity = 1.0e38;
	_xmin = _ymin = infinity;
	_xmax = _ymax = -infinity;

	read_from_structure(input);
}
 ...
 for(;;) 
    {
	  ...
	  int atoiToInsert = atoi( result.c_str() );
	  _nVertices.push_back( atoiToInsert );
...
/// Triangulate one countour
void BDMFile::read_from_structure(vector<pair<float, float>> input)
{
	double x, y;
	
	if( input.size() <= 2 ) {
		cout << "Not enough data for polygon" << endl;
		exit(1);
	}

	_ncontours = 1;
	_nVertices.push_back(input.size()); // ok
	...
	for( int i = 0; i < input.size(); i++ ) {
		...
		Pointbase* point = new Pointbase(i+1, x, y, INPUT);
		if( x > _xmax ) _xmax = x;
		if( x < _xmin ) _xmin = x;
		if( y > _ymax ) _ymax = y;
		if( y < _ymin ) _ymin = y;
		...
		if( i != 0 ) {
			_edges[l_id] = new Linebase(_points[i], _points[i+1], INPUT);
		}
		if (i == input.size() - 1){
			_edges[l_id] = new Linebase(_points[i+1], _points[1], INPUT);
		}
	}
	p_id = l_id;
	cout << "Tesselaton data loaded" << endl;
}
...
Polygon::Polygon(vector<pair<float, float>> input) : BDMFile(input)
{
	initializate();
	_debug = false;
}
...
vector<vector<pair<float, float>>> Polygon::saveIntoVector() {
	vector<vector<pair<float, float>>> result;
	Triangles::iterator itTriangles = _triangles.begin();

	for( ; itTriangles != _triangles.end(); itTriangles++ ) { // iterate through triangles
		vector<pair<float, float>> curTriangle;
		for( int i = 0; i < (*itTriangles).size(); i++ ) {
			map<unsigned int, Pointbase*>::iterator curVertice = _points.find((*itTriangles)[i]); // current vertice
			pair<float, float> verticeCoords = pair<float, float>((*curVertice).second->x, (*curVertice).second->y);
			curTriangle.push_back(verticeCoords);
		}
		result.push_back(curTriangle);
	}
	return result;
}